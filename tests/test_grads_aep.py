...
def test_gplvm_aep_gaussian():
    ...
    logZ, grad_all = lvm.objective_function(
        params, idxs, y_train, alpha=alpha)
    pp.pprint(logZ)
    pp.pprint(params)
    # pdb.set_trace()
...
def objective(params, params_args, obj, idxs, yb, alpha):
...
def gradient(params, params_args, obj, idxs, yb, alpha):
    params_dict = unflatten_dict(params, params_args)
    f, grad_dict = obj.objective_function(
        params_dict, idxs, yb, alpha=alpha)
...
def test_gplvm_aep_gaussian_scipy():
    ...
    logZ = objective(init_params_vec, params_args, lvm, idxs, y_train, alpha)
    pp.pprint(logZ)

    pp.pprint(check_grad(objective, gradient, init_params_vec, params_args, lvm, idxs, y_train, alpha))
