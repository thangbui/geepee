...
# load datasets
y_train = np.loadtxt('data/kink_train_%d_%.2f_%.2f.txt'%(index, dyn_noise, emi_noise))
...
model_aep.save_model('trained_models/kink_aep_model_index_%d_%.2f_%.2f_M_%d_alpha_%.4f_%s.pickle' % (index, dyn_noise, emi_noise, M, alpha, prop_mode))
