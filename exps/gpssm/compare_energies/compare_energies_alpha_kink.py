...
def plot_latent(model, y, plot_title='', color='k'):
    ...
    plt.savefig('/tmp/compare_energies_kink_'+plot_title+'.pdf', bbox_inches='tight')
...
def run_opt():
    ...
    final_params, costs = model_vfe.optimise(
        method='adam', adam_lr=0.008, maxiter=20000, 
        reinit_hypers=False, return_cost=True)
    np.savetxt('tmp/compare_energies_kink_vfe_obj.txt', costs, fmt='%.4e', delimiter=' ')
    plot_latent(model_vfe, y_train, 'VFE', color=tableau20[0])
    model_vfe.save_model('tmp/compare_energies_kink_model_vfe.pickle')
    ...
    for i, alpha in enumerate(alphas):
        for prop_mode in [PROP_MM, PROP_MC]:
            ...
            final_params, costs = model_aep.optimise(
                method='adam', alpha=alpha, adam_lr=0.008, maxiter=20000, 
                reinit_hypers=False, return_cost=True, prop_mode=prop_mode)
            np.savetxt('tmp/compare_energies_kink_aep_obj_%.4f_%s.txt' % (alpha, prop_mode), costs, fmt='%.4e', delimiter=' ')
            plot_latent(model_aep, y_train, 'AEP_%.4f_%s' % (alpha, prop_mode), color=tableau20[2 + i*2])
            model_aep.save_model('tmp/compare_energies_kink_model_aep_%.4f_%s.pickle' % (alpha, prop_mode))

def plot_opt():
    fig = plt.figure(figsize=figsize(1))
    vfe_obj = np.loadtxt('tmp/compare_energies_kink_vfe_obj.txt')
    ...
    for i, alpha in enumerate(alphas):
        for prop_mode in [PROP_MM, PROP_MC]:
            aep_obj = np.loadtxt('tmp/compare_energies_kink_aep_obj_%.4f_%s.txt' % (alpha, prop_mode))
    ...
    plt.xlim([1, 20000])
    plt.savefig('/tmp/compare_energies_kink.pdf', bbox_inches='tight')
    ...
    plt.savefig('/tmp/compare_energies_kink_zoomed.pdf', bbox_inches='tight')
