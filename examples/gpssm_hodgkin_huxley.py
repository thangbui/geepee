...
def plot_model_with_control(model, plot_title='', name_suffix=''):
	...
	for c_ind, c_val in enumerate(c_unique):
		...
		plt.ylim([mins[1], maxs[1]])
		plt.title(plot_title)
		plt.savefig('/tmp/hh_gpssm_dim_0'+name_suffix+'_c_%.2f.pdf'%c_val)
		...
		plt.ylim([mins[1], maxs[1]])
		plt.title(plot_title)
		plt.savefig('/tmp/hh_gpssm_dim_1'+name_suffix+'_c_%.2f.pdf'%c_val)
...
def plot_posterior_linear(params_fname, fig_fname, control=False, M=20):
	...
	for i in range(4):
		...
		plt.fill_between(t, mi+2*np.sqrt(vi), mi-2*np.sqrt(vi), color=cs[i], alpha=0.4)
...
def plot_prediction_gp(params_fname, fig_fname, M=20):
	...
	for i in range(4):
		...
		plt.fill_between(t, mi+2*np.sqrt(vi), mi-2*np.sqrt(vi), color=cs[i], alpha=0.4)
		plt.plot(t, mi, '-', color=cs[i])
		plt.fill_between(np.arange(T, 2*T), mip+2*np.sqrt(vip), mip-2*np.sqrt(vip), color=cs[i], alpha=0.4)
...
if __name__ == '__main__':
	...
	model_gp('/tmp/hh_gpssm_gp_with_control.pickle', control=True, M=M, alpha=alpha)
	plot_posterior_gp(
