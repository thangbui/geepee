...
for i, val in enumerate(m_ins):
    ...
    pdfs_app = mlab.normpdf(xplot, m_out_app, np.sqrt(v_out_app))
    axs_mm[i].plot(xplot, pdfs, color=tableau20[i*2], linewidth=2)
    axs_mm[i].plot(xplot, pdfs_app, '--', color=tableau20[i*2], linewidth=2)
...
plt.savefig('/tmp/prop_mm_approx.pdf', bbox_inches='tight')
